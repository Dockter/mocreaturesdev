        ...
        if (CustomSpawner.eventListeners) {
            ...
            for (EntitySpawnType entitySpawnType : environment.entitySpawnTypes.values()) {
                if (entitySpawnType.getChunkSpawnChance() > 0 && entitySpawnType.getSpawnCap() > 0) {
                    List<SpawnListEntry> customSpawnList = entitySpawnType.getBiomeSpawnList(Biome.getIdForBiome(event.getWorld().getBiome(new BlockPos(x, 0, z))));
                    if (customSpawnList != null) {
                        CustomSpawner.performWorldGenSpawning(entitySpawnType, event.getWorld(), event.getWorld().getBiome(new BlockPos(x, 0, z)), par1 + 8, par2 + 8, 16, 16, event.getRand(), customSpawnList, environment.worldGenCreatureSpawning);
                    }
                }
            }
        }
        ...
        if (CustomSpawner.eventListeners) {
            ...
            if (CustomSpawner.forceDespawns) {
                ...
                if (event.getEntityLiving() instanceof EntitySheep || event.getEntityLiving() instanceof EntityPig || event.getEntityLiving() instanceof EntityCow || event.getEntityLiving() instanceof EntityChicken) {
                    // check lightlevel
                    if (CustomDespawner.isValidDespawnLightLevel(event.getEntity(), event.getWorld(), CMSUtils.getEnvironment(event.getWorld()).minDespawnLightLevel, CMSUtils.getEnvironment(event.getWorld()).maxDespawnLightLevel)) {
                        return;
                    }
                }
                // Others
                ...
                if (event.getResult() == Result.ALLOW && CustomSpawner.debug) {
                    ...
                    CMSUtils.getEnvironment(event.getWorld()).envLog.logger.info("Forced Despawn of entity " + event.getEntityLiving() + " at " + x + ", " + y + ", " + z + ". To prevent forced despawns, use /moc forceDespawns false.");
                }
            }
