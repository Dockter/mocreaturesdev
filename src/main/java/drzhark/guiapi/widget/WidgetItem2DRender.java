...
public class WidgetItem2DRender extends Widget {
    ...
    static {
        try {
            ...
            try {
                ...
                System.out
                        .println("GuiAPI Warning: Unable to get Tessellator.isDrawing field! There will be a chance of crashes if you attempt to render a mod item!");
            }
    ...
    protected void paintWidget(GUI gui) {
        ...
        try {
            ...
            WidgetItem2DRender.itemRenderer.renderItemIntoGUI(stack, x, y);
            if (isDrawing(Tessellator.getInstance())) {
                //Yes, this IS stuff to work around 'bad' mods :D
                setDrawing(Tessellator.getInstance(), false);
            }
     ...
     *
     * @param renderID The ID you want this widget to render.
     */
    /*
     * public void setRenderID(int renderID) { if ((renderID >=
     * Item.itemsList.length) || (renderID < 0)) { throw new
     * IndexOutOfBoundsException( String.format(
     * "Render ID must be within the possible bounds of an Item ID! (%s - %s)",
     * 0, Item.itemsList.length - 1)); } if (renderStack == null) { renderStack
     * = new ItemStack(Item.getItemById(renderID), 0, 0); } renderStack.itemID =
     * renderID; }
     */
     ...
     */
    /*
     * public void setRenderStack(ItemStack stack) { if (stack == null) { throw
     * new IllegalArgumentException("stack cannot be null."); } if
     * ((stack.itemID >= Item.itemsList.length) || (stack.itemID < 0)) { throw
     * new IndexOutOfBoundsException( String.format(
     * "Render ID must be within the possible bounds of an Item ID! (%s - %s)",
     * 0, Item.itemsList.length - 1)); } renderStack = stack; }
     */
